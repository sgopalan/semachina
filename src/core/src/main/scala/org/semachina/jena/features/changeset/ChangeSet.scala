...
import org.semachina.jena.SemachinaDSL
import org.semachina.jena.ontology.impl.SemachinaIndividualAdapter
...
class ChangeSet {
  ...
  def create(implicit model: OntModel): Individual = {
    ...
    var extended: SemachinaOntModel = SemachinaDSL.toSemachinaOntModel(model)
