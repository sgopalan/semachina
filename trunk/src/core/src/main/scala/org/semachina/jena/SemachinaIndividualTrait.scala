...
import org.semachina.jena.impl.scala.{SemachinaIndividualAdapter, SemachinaOntModelAdapter}
import org.semachina.jena.SemachinaOntModelTrait._
...
object SemachinaIndividualTrait {

   def apply(individual: Individual) = asSemachinaIndividualTrait( individual )

   implicit def asSemachinaIndividualTrait(individual: Individual): SemachinaIndividualTrait = {
    if (individual.isInstanceOf[SemachinaIndividualTrait]) {
      return individual.asInstanceOf[SemachinaIndividualTrait]
    }
    else {
...
trait SemachinaIndividualTrait {
  ...
  def list(ontProperty: Property): ExtendedIterator[Individual] = {
    return WrappedIterator.create(getIndividual.listProperties(ontProperty).mapWith(stmtToObjectConverter))
  }
  ...
  protected def process(ontProperty: Property, value: Any, action: (Property, RDFNode) => Unit) {
    value match {
      ...
      case iterator: java.lang.Iterable[Any] => iterator.foreach{ process( ontProperty, _ , action ) }
