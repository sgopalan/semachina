...
import org.semachina.jena.{SemachinaOntModel, SemachinaFactory, TypedDatatypeProperty, SemachinaIndividual}
...
class DataValueWrapper[T](indiv:SemachinaIndividual, ontProperty:TypedDatatypeProperty[T]) {
  val factory = indiv.getOntModel.asInstanceOf[SemachinaOntModel].getFactory
  ...
  def <#[T] (value:T) = indiv.set( ontProperty, factory.createTypedLiteral( value ) )

  def <#[T] (values:Iterable[T]) =
    indiv.setAll( ontProperty, asIterable( values.collect{ case x : T => factory.createTypedLiteral( x ) } ) )

  def <<#[T] (value:T) = indiv.add( ontProperty, factory.createTypedLiteral( value ) )

  def <<#[T] (values:Iterable[T]) =
    indiv.addAll( ontProperty, asIterable( values.collect{ case x : T => factory.createTypedLiteral( x ) } ) )

  def -#[T] (value:T) =
    indiv.remove( ontProperty, factory.createTypedLiteral( value ) )

  def -#[T] (values:Iterable[T]) = 
    indiv.removeThese( ontProperty, asIterable( values.collect{ case x : T => factory.createTypedLiteral( x ) } ) )
}
