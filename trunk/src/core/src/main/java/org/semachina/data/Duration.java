...
public class Duration implements java.io.Serializable {
	...
	public void parse(String lexicalForm) {
		int position = 1;
        int timePosition = lexicalForm.indexOf("T");

        // P is required but P by itself is invalid
        if (lexicalForm.indexOf("P") == -1 || lexicalForm.equals("P")) {
            throw new IllegalArgumentException(
                    "bad lexicalForm" );//Messages.getMessage("badDuration"));
        }

        // if present, time cannot be empty
        if (lexicalForm.lastIndexOf("T") == lexicalForm.length() - 1) {
            throw new IllegalArgumentException(
            	    "bad duration" );//Messages.getMessage("badDuration"));
        }
        ...
        // parse time part
        if (timePosition != -1) {
            parseTime(lexicalForm.substring(timePosition + 1));
        } else {
            timePosition = lexicalForm.length();
        }

        // parse date part
        if (position != timePosition) {
            parseDate(lexicalForm.substring(position, timePosition));
        }	
	}
