...
public class MonthDay implements java.io.Serializable {
	...
	public void parseLexicalForm(String lexicalForm) {
        if (lexicalForm.length() < 6) {
            throw new NumberFormatException(
            		"bad month day" );
                    //Messages.getMessage("badMonthDay00"));
        }
        ...
        if ( lexicalForm.charAt(0) != '-' ||
        	 lexicalForm.charAt(1) != '-' ||
        	 lexicalForm.charAt(4) != '-' ) {
            throw new NumberFormatException(
            		"bad month day" );
                    //Messages.getMessage("badMonthDay00"));
        }

        setValue(Integer.parseInt(lexicalForm.substring(2,4)),
                 Integer.parseInt(lexicalForm.substring(5,7)),
                 lexicalForm.substring(7));
	}
